...
void CreatureTemplate::readObject(LuaObject* templateData) {
	...
	//RP
	LuaObject rpStatTable = templateData->getObjectField("rp_stats");
	if (rpStatTable.isValidTable()) {
		for (int i = 1; i <= rpStatTable.getTableSize(); ++i) {
			rp_stats.add(rpStatTable.getIntAt(i));
		}
	}

	rpStatTable.pop();

	LuaObject rpSkillTable = templateData->getObjectField("rp_skills");
	if (rpSkillTable.isValidTable()) {
		for (int i = 1; i <= rpSkillTable.getTableSize(); ++i) {
			rp_stats.add(rpSkillTable.getIntAt(i));
		}
	}

	rpSkillTable.pop();

	LuaObject rpPoolTable = templateData->getObjectField("rp_pools");
	if (rpPoolTable.isValidTable()) {
		for (int i = 1; i <= rpPoolTable.getTableSize(); ++i) {
			rp_stats.add(rpPoolTable.getIntAt(i));
		}
	}
