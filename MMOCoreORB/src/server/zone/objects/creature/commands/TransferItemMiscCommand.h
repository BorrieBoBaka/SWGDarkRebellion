...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		//creature->sendSystemMessage("Hello");
		...
		if(objectsParent != nullptr) {
			if(objectsParent->getStoredInt("dispenser_container") == 1) {
				return doTransferDispenser(creature, objectToTransfer, destinationObject);
			}
		}
	...
	int static doTransferDispenser(CreatureObject* creature, SceneObject* objectToTransfer, SceneObject* destinationObject) {
		
		Reference<SharedObjectTemplate*> shot = TemplateManager::instance()->getTemplate(objectToTransfer->getObjectTemplate()->getFullTemplateString().hashCode());
